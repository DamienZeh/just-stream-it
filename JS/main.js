...
var getMovies = async function (url,nameCarousel, numberMovie, numberImage) {    
  try {
      ...
      if (response.ok) {
          ...
          if (response.ok) {
              ...
              getDataMovie(allMovies, nameCarousel, numberMovie, numberImage);
              for (let movie in allMovies){ // test pour parcourir les données des films de la liste
                console.log(allMovies[movie].id)                             
              }
          }
          else {
...
var getDataMovie = async function (moviesData,nameCarousel, numberMovie, numberImage) {   
    while (indexMovie !=8){          
      ...
      console.log(moviesData[numberMovie].title)
      let title = document.querySelector('#titleMovie') //test pour nom de film dans la module box
      title.innerHTML = moviesData[numberMovie].title;
      indexMovie++;
      numberMovie++;
      numberImage+=1
...
getMovies(urlBestMovies,'#carousel1', 0, 1)
//getMovies(urlBestAnimationMovies,'#carousel2', 0, 1)
//getMovies(urlBestActionMovies,'#carousel3', 0, 1)
...
class Carousel {
    ...
    setStyle () {
      ...
      this.container.style.width = (ratio * 50) + "%"
    ...
    gotoItem (index, animation = true) {
      ...
      let translateX = index * -30 / this.items.length
      if (animation === false) {
